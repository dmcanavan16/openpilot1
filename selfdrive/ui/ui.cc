...
void update_model(UIState *s, 
  ...
  auto params = Params();
  static bool customRoadUIChecked = false;
  static bool isCustomRoadUI = false;
  static bool isUnlimitedLength = false;
  ...
  bool isCustomRoadUIUpdated = params.getBool("CustomRoadUIUpdated");
  // If CustomRoadUI has been updated, update the road UI
  if (isCustomRoadUIUpdated or !customRoadUIChecked) {
    isCustomRoadUI = params.getBool("CustomRoadUI");
    isUnlimitedLength = isCustomRoadUI && params.getBool("UnlimitedLength");
    lane_line_width = std::stof(params.get("LaneLinesWidth")) / 12 * 0.1524; // Convert from inches to meters
    path_edge_width = std::stof(params.get("PathEdgeWidth"));
    path_width = std::stof(params.get("PathWidth")) / 10 * 0.1524; // Convert from feet to meters
    road_edge_width = std::stof(params.get("RoadEdgesWidth")) / 12 * 0.1524; // Convert from inches to meters
    params.putBool("CustomRoadUIUpdated", false);
    customRoadUIChecked = true;
  }
