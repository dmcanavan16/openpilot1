...
AbstractAlert::AbstractAlert(bool hasRebootBtn, QWidget *parent) : QFrame(parent) {
  ...
  if (hasRebootBtn) {
    ...
    QObject::connect(rebootBtn, &QPushButton::clicked, [=]() {
      ...
      QString currentDateTimeStr = currentDateTime.toString("MMMM d'th', yyyy - h:mma");
      params.put("Updated", currentDateTimeStr.toStdString());
      QObject::connect(rebootBtn, &QPushButton::clicked, [=]() { QProcess::execute("rm -f /data/openpilot/prebuilt"); }); // Remove the prebuilt file when installing updates
      Hardware::reboot();
    });
  }
