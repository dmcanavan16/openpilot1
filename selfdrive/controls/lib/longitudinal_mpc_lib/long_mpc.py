...
class LongitudinalMpc:
  ...
  def set_weights(self, prev_accel_constraint=True, v_lead0=0, v_lead1=0):
    if self.mode == 'acc':
      ...
      cost_weights = [X_EGO_OBSTACLE_COST, X_EGO_COST, V_EGO_COST, A_EGO_COST, a_change_cost * cost_mulitpliers[0], J_EGO_COST * cost_mulitpliers[1]] if self.adjustable_follow else [X_EGO_OBSTACLE_COST, X_EGO_COST, V_EGO_COST, A_EGO_COST, a_change_cost, J_EGO_COST]
      constraint_cost_weights = [LIMIT_COST, LIMIT_COST, LIMIT_COST, DANGER_ZONE_COST * cost_mulitpliers[2]] if self.adjustable_follow else [LIMIT_COST, LIMIT_COST, LIMIT_COST, DANGER_ZONE_COST]
  ...
  def update_TF(self, carstate):
    if self.adjustable_follow:
      ...
      profile_key = carstate.distanceLines if self.adjustable_follow else 2
      profile = distance_profiles.get(profile_key, distance_profiles[1])

      # Slowly increase the following distance when the value of distanceLines goes higher
      if profile_key <= self.prev_profile_key:
        ...
        rate = 0.01 if profile_key == 2 else 0.001
  ...
  def update(self, carstate, radarstate, v_cruise, x, v, a, j, prev_accel_constraint):
    ...
    if self.mode == 'acc':
      ...
      v_cruise_clipped = np.clip(v_cruise * np.ones(N+1),
                                 v_lower,
                                 v_upper)
      cruise_obstacle = np.cumsum(T_DIFFS * v_cruise_clipped) + get_safe_obstacle_distance(v_cruise_clipped, self.desired_TF) if self.adjustable_follow else np.cumsum(T_DIFFS * v_cruise_clipped) + get_safe_obstacle_distance(v_cruise_clipped)
